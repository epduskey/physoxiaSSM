# Hypoxia physiological analysis for BP

# Created: September 2, 2022
# Last modified: March 2, 2023 by EPD
...
species_mat = sp_mat(rmax = c(4.6e+08,9.7e+08,2.0e+11,1.9e+10))
...
species_params(params_test)$z0 = c(0.06, 0.21, 0.30, 0.25)
species_params(params_test)$gamma = c(1.0e-10, 4.2e-11, 2.3e-11, 3.0e-11)
...
# Fit yield
par_yield = c(0.06, 0.21, 0.30, 0.25)
species_params(params_tune)$gamma = c(1.0e-10, 4.2e-11, 2.3e-11, 3.0e-11)
...
params_test = params_tune
species_params(params_test)$R_max = c(1.2e+08,4.2e+08,5.1e+11,2.3e+10)
sim_test = project(params_test, t_max = t_tune, effort = effort_hypoxia)
plotBiomassObservedVsModel(sim_test)
plotBiomass(sim_test)

# Fit biomass and growth
par_bg = c(log10(c(1.2e+08,4.2e+08,5.1e+11,2.3e+10)), log10(species_params(params_tune)$gamma))
...
benthic.optim = optimParallel(par = par_tg_benthic, params = params_time, benthic_growth, yr = yr_tg, t = t_tune, effort = effort_time, lower = c(0,-10), method = "L-BFGS-B", control = list(factr=1e10), parallel = list(cl=cl,loginfo=T))
stopCluster(cl)
...
save(object = bp_fit, file = "Sensitivity/Three/Calibration/bp_fit.rda")

# Save the final objects
save(object = params_tune, file = "Sensitivity/Three/Calibration/params_tune_bp.rda")
save(object = params_time, file = "Sensitivity/Three/Calibration/params_time_bp.rda")


########## X. Analysis ##########

# Load the final objects
load("Sensitivity/Three/Calibration/params_tune_bp.rda")
load("Sensitivity/Three/Calibration/params_time_bp.rda")
...
save(object = params_full, file = "Sensitivity/Three/Calibration/params_full_bp.rda")