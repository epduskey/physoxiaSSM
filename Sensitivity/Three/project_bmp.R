...
# Last modified: March 1, 2023 by EPD
...
species_mat = sp_mat(rmax = c(4.9e+08,9.5e+08,2.0e+11,1.9e+10))
...
species_params(params_test)$z0 = c(0.06, 0.21, 0.30, 0.26)
species_params(params_test)$gamma = c(1.0e-10, 4.3e-11, 2.3e-11, 3.1e-11)
...
# Fit yield
par_yield = c(0.06, 0.21, 0.30, 0.26)
species_params(params_tune)$gamma = c(1.0e-10, 4.3e-11, 2.3e-11, 3.1e-11)
...
params_test = params_tune
species_params(params_test)$R_max = c(1.3e+08,4.1e+08,5.1e+11,2.4e+10)
sim_test = project(params_test, t_max = t_tune, effort = effort_hypoxia)
plotBiomassObservedVsModel(sim_test)
plotBiomass(sim_test)

# Fit biomass and growth
par_bg = c(log10(c(1.3e+08,4.1e+08,5.1e+11,2.4e+10)), log10(species_params(params_tune)$gamma))
...
bg.optim = optimParallel(par = par_bg, params = params_tune, fit_bg, dat = dat_bg, yr = yr_bg, t = t_bg, effort = effort_hypoxia, lower = c(5,5,5,5,-15,-15,-15,-15), upper = c(15,15,15,15,-5,-5,-5,-5), method = "L-BFGS-B", parallel = list(cl=cl,loginfo=T))
stopCluster(cl)
...
tg.optim.cod.phys = optimParallel(par = par_tg_cod_phys, params = params_time, species = "cod", time_growth, yr = yr_tg, t = t_tune, effort = effort_time, lower = rep(c(0,-10),5), method = "L-BFGS-B", control = list(factr=1e10), parallel = list(cl=cl,loginfo=T))
stopCluster(cl)
...
tg.optim.flounder.phys = optimParallel(par = par_tg_flounder_phys, params = params_time, species = "flounder", time_growth, yr = yr_tg, t = t_tune, effort = effort_time, lower = rep(c(0,-10),5), method = "L-BFGS-B", parallel = list(cl=cl,loginfo=T))
stopCluster(cl)
...
benthic.optim = optimParallel(par = par_tg_benthic, params = params_time, benthic_growth, yr = yr_tg, t = t_tune, effort = effort_time, lower = c(0,-10), method = "L-BFGS-B", parallel = list(cl=cl,loginfo=T))
stopCluster(cl)
...
save(object = bmp_fit, file = "Sensitivity/Three/Calibration/bmp_fit.rda")

# Save the final objects
save(object = params_tune, file = "Sensitivity/Three/Calibration/params_tune_bmp.rda")
save(object = params_time, file = "Sensitivity/Three/Calibration/params_time_bmp.rda")


########## X. Analysis ##########

# Load the final objects
load("Sensitivity/Three/Calibration/params_tune_bmp.rda")
load("Sensitivity/Three/Calibration/params_time_bmp.rda")
...
save(object = params_full, file = "Sensitivity/Three/Calibration/params_full_bmp.rda")