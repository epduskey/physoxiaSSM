# Hypoxia physiological analysis for MP
...
species_mat = sp_mat(rmax = c(4.2e+08,7.6e+08,2.0e+11,1.9e+10))
...
species_params(params_test)$z0 = c(0.05, 0.19, 0.31, 0.26)
species_params(params_test)$gamma = c(9.6e-11, 3.6e-11, 2.4e-11, 3.0e-11)
...
# Fit yield
par_yield = c(0.05, 0.19, 0.31, 0.26)
species_params(params_tune)$gamma = c(9.6e-11, 3.6e-11, 2.4e-11, 3.0e-11)
...
params_test = params_tune
species_params(params_test)$R_max = c(1.2e+08,3.4e+08,5.3e+11,2.4e+10)
sim_test = project(params_test, t_max = t_tune, effort = effort_hypoxia)
plotBiomassObservedVsModel(sim_test)
plotBiomass(sim_test)

# Fit biomass and growth
par_bg = c(log10(c(1.2e+08,3.4e+08,5.3e+11,2.4e+10)), log10(species_params(params_tune)$gamma))
...
tg.optim.flounder.phys = optimParallel(par = par_tg_flounder_phys, params = params_time, species = "flounder", time_growth, yr = yr_tg, t = t_tune, effort = effort_time, lower = rep(c(0,-10),5), method = "L-BFGS-B", control = list(factr=1e12), parallel = list(cl=cl,loginfo=T))
stopCluster(cl)
...
cod.mort.optim = optimParallel(par = par_cod_mort, params = params_time, dat = dat_yield, species = "cod", time_yield, yr = yr_mort, t = t_mort, effort = effort_time, lower = c(0,0), method = "L-BFGS-B", control = list(factr=1e14), parallel = list(cl=cl,loginfo=T))
stopCluster(cl)
...
save(object = mp_fit, file = "Sensitivity/Two/Calibration/mp_fit.rda")

# Save the final objects
save(object = params_tune, file = "Sensitivity/Two/Calibration/params_tune_mp.rda")
save(object = params_time, file = "Sensitivity/Two/Calibration/params_time_mp.rda")


########## X. Analysis ##########

# Load the final objects
load("Sensitivity/Two/Calibration/params_tune_mp.rda")
load("Sensitivity/Two/Calibration/params_time_mp.rda")
...
save(object = params_full, file = "Sensitivity/Two/Calibration/params_full_mp.rda")