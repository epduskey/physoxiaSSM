# Run hypoxia physiological model sensitivity analysis for BOMP

# Created: January 31, 2023
# Last modified: February 4, 2023 by EPD
...
species_mat = sp_mat(rmax = c(3.2e+08,9.5e+08,2.1e+11,1.9e+10))
...
species_params(params_test)$z0 = c(0.05, 0.25, 0.30, 0.24)
species_params(params_test)$gamma = c(8.0e-11, 4.4e-11, 2.3e-11, 3.0e-11)
...
# Fit occupancy
species_params(params_tune)$z0 = c(0.05, 0.25, 0.30, 0.24)
species_params(params_tune)$gamma = c(8.0e-11, 4.4e-11, 2.3e-11, 3.0e-11)
par_occ = c(0.8, 0.1)
...
params_tune = oxy_sensitivity(params_tune, 
				U_hab = c(occ.optim$par[1], U_hab[2:4]),
				a_hab = c(occ.optim$par[2], a_hab[2:4]),
				U_search = U_search,
				a_search = a_search,
				U_maxin = U_maxin,
				a_maxin = a_maxin,
...
params_time = rate_scale(params_tune, t_tune)
params_time = mscale(params_tune, t_tune)
...
params_test = params_tune
species_params(params_test)$R_max = c(1.0e+08,4.5e+08,4.9e+11,2.3e+10)
sim_test = project(params_test, t_max = t_tune, effort = effort_hypoxia)
plotBiomassObservedVsModel(sim_test)
plotBiomass(sim_test)

# Fit biomass and growth
par_bg = c(log10(c(1.0e+08,4.5e+08,4.9e+11,2.3e+10)), log10(species_params(params_tune)$gamma))
...
benthic.optim = optimParallel(par = par_tg_benthic, params = params_time, benthic_growth, yr = yr_tg, t = t_tune, effort = effort_time, lower = c(0,-10), method = "L-BFGS-B", control = list(factr=1e13), parallel = list(cl=cl,loginfo=T))
stopCluster(cl)
...
save(object = bomp_fit, file = "Sensitivity/One/Calibration/bomp_fit.rda")

# Save the final objects
save(object = params_tune, file = "Sensitivity/One/Calibration/params_tune_bomp.rda")
save(object = params_time, file = "Sensitivity/One/Calibration/params_time_bomp.rda")


########## X. Analysis ##########

# Load the final objects
load("Sensitivity/One/Calibration/params_tune_bomp.rda")
load("Sensitivity/One/Calibration/params_time_bomp.rda")
...
save(object = params_full, file = "Sensitivity/One/Calibration/params_full_bomp.rda")