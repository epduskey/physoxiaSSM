# Hypoxia physiological analysis for OMP

# Created: September 2, 2022
# Last modified: February 23, 2023 by EPD
...
species_mat = sp_mat(rmax = c(3.0e+08,8.5e+08,2.1e+11,1.9e+10))
...
species_params(params_test)$z0 = c(0.04, 0.23, 0.30, 0.24)
species_params(params_test)$gamma = c(7.6e-11, 4.0e-11, 2.3e-11, 3.0e-11)
...
# Fit occupancy
species_params(params_tune)$z0 = c(0.04, 0.23, 0.30, 0.24)
species_params(params_tune)$gamma = c(7.6e-11, 4.0e-11, 2.3e-11, 3.0e-11)
par_occ = c(0.8, 0.1)
...
yield.optim = optimParallel(par = par_yield, fn = fit_yield, params = params_tune, dat = dat_yield, yr = yr_yield, t = t_yield, effort = effort_hypoxia, lower = c(0,0,0,0), upper = c(1,1,1,1), method = "L-BFGS-B", control = list(factr=1e9), parallel=list(cl=cl,loginfo=T))
stopCluster(cl)
...
params_test = params_tune
species_params(params_test)$R_max = c(9.8e+07,4.1e+08,5.1e+11,2.3e+10)
sim_test = project(params_test, t_max = t_tune, effort = effort_hypoxia)
plotBiomassObservedVsModel(sim_test)
plotBiomass(sim_test)

# Fit biomass and growth
par_bg = c(log10(c(9.8e+07,4.1e+08,5.1e+11,2.3e+10)), log10(species_params(params_tune)$gamma))
...
tg.optim.cod.phys = optimParallel(par = par_tg_cod_phys, params = params_time, species = "cod", time_growth, yr = yr_tg, t = t_tune, effort = effort_time, lower = rep(c(0,-10),5), method = "L-BFGS-B", control = list(factr=1e9), parallel = list(cl=cl,loginfo=T))
stopCluster(cl)
...
save(object = omp_fit, file = "Sensitivity/One/Calibration/omp_fit.rda")

# Save the final objects
save(object = params_tune, file = "Sensitivity/One/Calibration/params_tune_omp.rda")
save(object = params_time, file = "Sensitivity/One/Calibration/params_time_omp.rda")


########## X. Analysis ##########

# Load the final objects
load("Sensitivity/One/Calibration/params_tune_omp.rda")
load("Sensitivity/One/Calibration/params_time_omp.rda")
...
save(object = params_full, file = "Sensitivity/One/Calibration/params_full_omp.rda")