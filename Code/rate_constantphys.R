...
hypoxiaEncounter = function(params, n, n_pp, n_other, t, ...) {
	...
	encounter = params@other_params$rate_scale[t+1,,] * encounter
	return(encounter)
}
...
hypoxiaPredRate <- function(params, n, n_pp, n_other, t, feeding_level, ...) {
	...
	if (!is.null(comment(params@pred_kernel))) {
		...
		pred_rate$benthic = sweep(params@pred_kernel, c(1,2),
					(1 - feeding_level) * 
					(params@search_vol * params@other_params$rate_scale[t+1,,]) * 
		...
		Q_benthic[, idx_sp] = sweep( (1 - feeding_level) * (params@search_vol * params@other_params$rate_scale[t+1,,]) * (n * params@other_params$benthic_occupancy[t+1,,]), 2, params@dw, "*")
		Q_pelagic[, idx_sp] = sweep( (1 - feeding_level) * (params@search_vol * params@other_params$rate_scale[t+1,,]) * (n * (1-params@other_params$benthic_occupancy[t+1,,])), 2, params@dw, "*")
...
hypoxiaEReproAndGrowth <- function(params, n, n_pp, n_other, t, encounter, feeding_level, ...) {
	alpha = params@species_params$alpha * params@other_params$rate_scale[t+1,,]
