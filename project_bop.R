...
species_mat = sp_mat(rmax = c(3.6e+08,9.6e+08,2.1e+11,1.9e+10))
...
species_params(params_test)$gamma = c(9.0e-11, 4.4e-11, 2.3e-11, 3.0e-11)
...
species_params(params_tune)$gamma = c(9.0e-11, 4.4e-11, 2.3e-11, 3.0e-11)
par_occ = c(0.8, 0.1)
...
params_test = params_tune
species_params(params_test)$R_max = c(1.0e+08,4.3e+08,5.0e+11,2.3e+10)
sim_test = project(params_test, t_max = t_tune, effort = effort_hypoxia)
plotBiomassObservedVsModel(sim_test)
plotBiomass(sim_test)

# Fit biomass and growth
par_bg = c(log10(c(1.0e+08,4.3e+08,5.0e+11,2.3e+10)), log10(species_params(params_tune)$gamma))
...
benthic.optim = optimParallel(par = par_tg_benthic, params = params_time, benthic_growth, yr = yr_tg, t = t_tune, effort = effort_time, lower = c(0,-10), method = "L-BFGS-B", control = list(pgtol=1e3), parallel = list(cl=cl,loginfo=T))
stopCluster(cl)
...
save(object = bop_fit, file = "Calibration/bop_fit.rda")

# Save the final objects
save(object = params_tune, file = "Calibration/params_tune_bop.rda")
save(object = params_time, file = "Calibration/params_time_bop.rda")


########## X. Analysis ##########

# Load the final objects
load("Calibration/params_tune_bop.rda")
load("Calibration/params_time_bop.rda")
...
save(object = params_full, file = "Calibration/params_full_bop.rda")