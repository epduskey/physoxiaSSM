...
species_mat = sp_mat(rmax = c(3.7e+08,8.5e+08,2.0e+11,1.9e+10))
...
species_params(params_test)$gamma = c(9.4e-11, 4.0e-11, 2.5e-11, 3.0e-11)
...
species_params(params_tune)$gamma = c(9.4e-11, 4.0e-11, 2.5e-11, 3.0e-11)
...
params_test = params_tune
species_params(params_test)$R_max = c(1.1e+08,4.0e+08,5.3e+11,2.4e+10)
sim_test = project(params_test, t_max = t_tune, effort = effort_hypoxia)
plotBiomassObservedVsModel(sim_test)
plotBiomass(sim_test)

# Fit biomass and growth
par_bg = c(log10(c(1.1e+08,4.0e+08,5.3e+11,2.4e+10)), log10(species_params(params_tune)$gamma))
...
tg.optim.flounder.phys = optimParallel(par = par_tg_flounder_phys, params = params_time, species = "flounder", time_growth, yr = yr_tg, t = t_tune, effort = effort_time, lower = rep(c(0,-10),5), method = "L-BFGS-B", control = list(factr=1e9), parallel = list(cl=cl,loginfo=T))
stopCluster(cl)
...
save(object = p_fit, file = "Calibration/p_fit.rda")

# Save the final objects
save(object = params_tune, file = "Calibration/params_tune_p.rda")
save(object = params_time, file = "Calibration/params_time_p.rda")


########## X. Analysis ##########

# Load the final objects
load("Calibration/params_tune_p.rda")
load("Calibration/params_time_p.rda")
...
save(object = params_full, file = "Calibration/params_full_p.rda")