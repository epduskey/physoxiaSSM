# Run hypoxia model sensitivity analysis for MP
...
species_mat = sp_mat(rmax = c(3.8e+08,9.0e+08,2.0e+11,1.9e+10))
...
species_params(params_test)$z0 = c(0.05, 0.22, 0.32, 0.26)
...
# Fit yield
par_yield = c(0.05, 0.22, 0.32, 0.26)
...
params_test = params_tune
species_params(params_test)$R_max = c(1.1e+08,4.1e+08,5.3e+11,2.4e+10)
sim_test = project(params_test, t_max = t_tune, effort = effort_hypoxia)
plotBiomassObservedVsModel(sim_test)
plotBiomass(sim_test)

# Fit biomass and growth
par_bg = c(log10(c(1.1e+08,4.1e+08,5.3e+11,2.4e+10)), log10(species_params(params_tune)$gamma))
...
bg.optim = optimParallel(par = par_bg, params = params_tune, fit_bg, dat = dat_bg, yr = yr_bg, t = t_bg, effort = effort_hypoxia, lower = c(5,5,5,5,-15,-15,-15,-15), upper = c(15,15,15,15,-5,-5,-5,-5), method = "L-BFGS-B", control = list(factr=1e14), parallel = list(cl=cl,loginfo=T))
stopCluster(cl)
