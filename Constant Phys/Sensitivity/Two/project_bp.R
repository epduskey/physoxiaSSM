...
# Last modified: December 26, 2022 by EPD
...
species_mat = sp_mat(rmax = c(3.8e+08,9.7e+08,2.0e+11,1.9e+10))
...
species_params(params_test)$z0 = c(0.06, 0.23, 0.33, 0.26)
species_params(params_test)$gamma = c(9.3e-11, 4.2e-11, 2.6e-11, 3.0e-11)
...
# Fit yield
par_yield = c(0.06, 0.23, 0.33, 0.26)
species_params(params_tune)$gamma = c(9.3e-11, 4.2e-11, 2.6e-11, 3.0e-11)
...
params_test = params_tune
species_params(params_test)$R_max = c(1.1e+08,4.3e+08,5.4e+11,2.4e+10)
sim_test = project(params_test, t_max = t_tune, effort = effort_hypoxia)
plotBiomassObservedVsModel(sim_test)
plotBiomass(sim_test)

# Fit biomass and growth
par_bg = c(log10(c(1.1e+08,4.3e+08,5.4e+11,2.4e+10)), log10(species_params(params_tune)$gamma))
...
benthic.optim = optimParallel(par = par_tg_benthic, params = params_time, benthic_growth, yr = yr_tg, t = t_tune, effort = effort_time, lower = c(0,-10), method = "L-BFGS-B", control = list(factr = 1e8), parallel = list(cl=cl,loginfo=T))
stopCluster(cl)
