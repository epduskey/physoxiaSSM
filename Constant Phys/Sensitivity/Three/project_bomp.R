...
species_mat = sp_mat(rmax = c(6.0e+08,9.3e+08,2.0e+11,1.9e+10))
...
species_params(params_test)$z0 = c(0.09, 0.20, 0.30, 0.25)
species_params(params_test)$gamma = c(1.2e-10, 4.1e-11, 2.3e-11, 3.0e-11)
...
# Fit occupancy
species_params(params_tune)$z0 = c(0.09, 0.20, 0.30, 0.25)
species_params(params_tune)$gamma = c(1.2e-10, 4.1e-11, 2.3e-11, 3.0e-11)
...
params_test = params_tune
species_params(params_test)$R_max = c(1.5e+08,4.0e+08,5.0e+11,2.3e+10)
sim_test = project(params_test, t_max = t_tune, effort = effort_hypoxia)
plotBiomassObservedVsModel(sim_test)
plotBiomass(sim_test)

# Fit biomass and growth
par_bg = c(log10(c(1.5e+08,4.0e+08,5.0e+11,2.3e+10)), log10(species_params(params_tune)$gamma))
...
tg.optim.flounder.phys = optimParallel(par = par_tg_flounder_phys, params = params_time, species = "flounder", time_growth, yr = yr_tg, t = t_tune, effort = effort_time, lower = c(0,-10,0,-10), method = "L-BFGS-B", control = list(factr=1e10), parallel = list(cl=cl,loginfo=T))
stopCluster(cl)
