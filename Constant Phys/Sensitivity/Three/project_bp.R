...
# Last modified: December 29, 2022 by EPD
...
species_params(params_test)$z0 = c(0.08, 0.22, 0.32, 0.25)
species_params(params_test)$gamma = c(1.2e-10, 4.1e-11, 2.6e-11, 3.0e-11)
...
# Fit yield
par_yield = c(0.08, 0.22, 0.32, 0.25)
species_params(params_tune)$gamma = c(1.2e-10, 4.1e-11, 2.6e-11, 3.0e-11)
...
params_test = params_tune
species_params(params_test)$R_max = c(1.3e+08,4.1e+08,5.1e+11,2.3e+10)
sim_test = project(params_test, t_max = t_tune, effort = effort_hypoxia)
plotBiomassObservedVsModel(sim_test)
plotBiomass(sim_test)

# Fit biomass and growth
par_bg = c(log10(c(1.3e+08,4.1e+08,5.1e+11,2.3e+10)), log10(species_params(params_tune)$gamma))
...
tg.optim.cod.phys = optimParallel(par = par_tg_cod_phys, params = params_time, species = "cod", time_growth, yr = yr_tg, t = t_tune, effort = effort_time, lower = c(0,-10,0,-10), method = "L-BFGS-B", control = list(factr=1e8), parallel = list(cl=cl,loginfo=T))
stopCluster(cl)
