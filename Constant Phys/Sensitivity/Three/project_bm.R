# Run hypoxia model sensitivity analysis for BM
...
species_mat = sp_mat(rmax = c(2.2e+08,7.6e+08,2.0e+11,1.9e+10))
...
params@other_params$rate_scale[,,] = 1
...
species_params(params_test)$z0 = c(0.00, 0.22, 0.34, 0.27)
species_params(params_test)$gamma = c(5.2e-11, 3.4e-11, 2.5e-11, 3.1e-11)
...
# Fit yield
par_yield = c(0.00, 0.22, 0.34, 0.27)
species_params(params_tune)$gamma = c(5.2e-11, 3.4e-11, 2.5e-11, 3.1e-11)
...
params_test = params_tune
species_params(params_test)$R_max = c(8.3e+07,3.9e+08,6.0e+11,2.5e+10)
sim_test = project(params_test, t_max = t_tune, effort = effort_hypoxia)
plotBiomassObservedVsModel(sim_test)
plotBiomass(sim_test)

# Fit biomass and growth
par_bg = c(log10(c(8.3e+07,3.9e+08,6.0e+11,2.5e+10)), log10(species_params(params_tune)$gamma))
...
plotGrowthCurves(sim_tune, species = "herring", max_age = 13)
plotBiomassObservedVsModel(sim_tune)
plotYieldObservedVsModel(sim_tune, 90, yr_yield)
...
save(object = bm_fit, file = "Sensitivity/Three/Calibration/bm_fit.rda")

# Save the final objects
save(object = params_tune, file = "Sensitivity/Three/Calibration/params_tune_bm.rda")
save(object = params_time, file = "Sensitivity/Three/Calibration/params_time_bm.rda")


########## X. Analysis ##########

# Load the final objects
load("Sensitivity/Three/Calibration/params_tune_bm.rda")
load("Sensitivity/Three/Calibration/params_time_bm.rda")
...
save(object = params_full, file = "Sensitivity/Three/Calibration/params_full_bm.rda")