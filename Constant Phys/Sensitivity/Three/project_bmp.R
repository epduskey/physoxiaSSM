...
# Last modified: December 27, 2022 by EPD
...
species_mat = sp_mat(rmax = c(6.1e+08,9.6e+08,2.0e+11,1.9e+10))
...
species_params(params_test)$z0 = c(0.09, 0.21, 0.30, 0.25)
species_params(params_test)$gamma = c(1.2e-10, 4.0e-11, 2.3e-11, 3.0e-11)
...
# Fit yield
par_yield = c(0.09, 0.21, 0.30, 0.25)
species_params(params_tune)$gamma = c(1.2e-10, 4.0e-11, 2.3e-11, 3.0e-11)
...
params_test = params_tune
species_params(params_test)$R_max = c(1.5e+08,4.0e+08,4.9e+11,2.3e+10)
sim_test = project(params_test, t_max = t_tune, effort = effort_hypoxia)
plotBiomassObservedVsModel(sim_test)
plotBiomass(sim_test)

# Fit biomass and growth
par_bg = c(log10(c(1.5e+08,4.0e+08,4.9e+11,2.3e+10)), log10(species_params(params_tune)$gamma))
...
tg.optim.cod.phys = optimParallel(par = par_tg_cod_phys, params = params_time, species = "cod", time_growth, yr = yr_tg, t = t_tune, effort = effort_time, lower = c(0,-10,0,-10), method = "L-BFGS-B", control = list(factr=1e9), parallel = list(cl=cl,loginfo=T))
stopCluster(cl)
...
tg.optim.flounder.phys = optimParallel(par = par_tg_flounder_phys, params = params_time, species = "flounder", time_growth, yr = yr_tg, t = t_tune, effort = effort_time, lower = c(0,-10,0,-10), method = "L-BFGS-B", parallel = list(cl=cl,loginfo=T))
stopCluster(cl)
