# Run hypoxia model sensitivity analysis for BMP

# Created: November 27, 2021
# Last modified: December 22, 2022 by EPD
...
species_mat = sp_mat(rmax = c(4.8e+08,9.9e+08,2.0e+11,1.9e+10))
...
species_params(params_test)$z0 = c(0.08, 0.21, 0.30, 0.26)
...
# Fit yield
par_yield = c(0.08, 0.21, 0.30, 0.26)
...
params_test = params_tune
species_params(params_test)$R_max = c(1.3e+08,4.3e+08,5.1e+11,2.4e+10)
sim_test = project(params_test, t_max = t_tune, effort = effort_hypoxia)
plotBiomassObservedVsModel(sim_test)
plotBiomass(sim_test)

# Fit biomass and growth
par_bg = c(log10(c(1.3e+08,4.3e+08,5.1e+11,2.4e+10)), log10(species_params(params_tune)$gamma))
...
tg.optim.cod.phys = optimParallel(par = par_tg_cod_phys, params = params_time, species = "cod", time_growth, yr = yr_tg, t = t_tune, effort = effort_time, lower = c(0,-10,0,-10), method = "L-BFGS-B", control = list(factr=1e10), parallel = list(cl=cl,loginfo=T))
stopCluster(cl)
...
tg.optim.flounder.phys = optimParallel(par = par_tg_flounder_phys, params = params_time, species = "flounder", time_growth, yr = yr_tg, t = t_tune, effort = effort_time, lower = c(0,-10,0,-10), method = "L-BFGS-B", control = list(factr=1e12), parallel = list(cl=cl,loginfo=T))
stopCluster(cl)
