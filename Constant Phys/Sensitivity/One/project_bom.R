# Run hypoxia model sensitivity analysis for BOM
...
species_mat = sp_mat(rmax = c(2.0e+08,7.5e+08,2.0e+11,1.9e+10))
...
species_params(params_test)$z0 = c(0.00, 0.22, 0.32, 0.27)
species_params(params_test)$gamma = c(5.0e-11, 3.4e-11, 2.3e-11, 3.0e-11)
...
# Fit occupancy
species_params(params_tune)$z0 = c(0.00, 0.22, 0.32, 0.27)
species_params(params_tune)$gamma = c(5.0e-11, 3.4e-11, 2.3e-11, 3.0e-11)
...
yield.optim = optimParallel(par = par_yield, fn = fit_yield, params = params_tune, dat = dat_yield, yr = yr_yield, t = t_yield, effort = effort_hypoxia, lower = c(0,0,0,0), upper = c(1,1,1,1), method = "L-BFGS-B", control = list(factr=1e10), parallel=list(cl=cl,loginfo=T))
stopCluster(cl)
...
params_test = params_tune
species_params(params_test)$R_max = c(7.7e+07,4.0e+08,5.9e+11,2.5e+10)
sim_test = project(params_test, t_max = t_tune, effort = effort_hypoxia)
plotBiomassObservedVsModel(sim_test)
plotBiomass(sim_test)

# Fit biomass and growth
par_bg = c(log10(c(7.7e+07,4.0e+08,5.9e+11,2.5e+10)), log10(species_params(params_tune)$gamma))
...
save(object = bom_fit, file = "Sensitivity/One/Calibration/bom_fit.rda")

# Save the final objects
save(object = params_tune, file = "Sensitivity/One/Calibration/params_tune_bom.rda")
save(object = params_time, file = "Sensitivity/One/Calibration/params_time_bom.rda")


########## X. Analysis ##########

# Load the final objects
load("Sensitivity/One/Calibration/params_tune_bom.rda")
load("Sensitivity/One/Calibration/params_time_bom.rda")
...
save(object = params_full, file = "Sensitivity/One/Calibration/params_full_bom.rda")